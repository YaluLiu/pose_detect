...
parser.add_argument('--gap', type=int, default=8,
                    help='detect once every gap frames')
"""----------------------------- Tracking options -----------------------------"""
...
def check_input():
    # for wecam
    if len(args.webcam) > 0:
        args.detbatch = 1
        if len(args.webcam) <= 2:
            return 'webcam', int(args.webcam)
        else:
            return 'webcam', args.webcam
...
if __name__ == "__main__":
    ...
    try:
        for i in im_names_desc:
            ...
            if args.profile:
                # TQDM
                avg_det_time = np.mean(runtime_profile['dt'])
                avg_pose_time = np.mean(runtime_profile['pt'])
                avg_post_process_time = np.mean(runtime_profile['pn'])
                avg_all_time = avg_det_time + avg_pose_time + avg_post_process_time
                im_names_desc.set_description(
                    'det time: {dt:.4f} | pose time: {pt:.4f} | post processing: {pn:.4f} | fps :{fps:.2f}'
                    .format(dt=avg_det_time, pt=avg_pose_time, pn=avg_post_process_time, fps = 1/avg_all_time)
                )
        print_finish_info()
